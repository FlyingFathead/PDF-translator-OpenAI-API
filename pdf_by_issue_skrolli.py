...
def process_pdf_directory(directory):
    ...
    for pdf_file in glob.glob(os.path.join(directory, '*.pdf')):
        ...
        formatted_issue = format_filename(os.path.basename(pdf_file))

        # Ensure the issue information is correctly structured in JSON
        json_data = {
            "issue": formatted_issue,  # Issue data taken from the filename
            "pages": pages
        }

        json_filename = os.path.join(output_dir, formatted_issue + '.json')
